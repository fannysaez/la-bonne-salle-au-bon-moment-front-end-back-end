...
export default function FormCompte() {
  ...
  const nextStep = async () => {
    ...
    const valid = await trigger(fields[mobileStep]);
    if (valid) setMobileStep((s) => s + 1);
  };
        ...
        <div className="flex items-center justify-center py-20">
          <div className="card p-10 flex flex-col items-center text-center max-w-md">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setSuccess(false); setMobileStep(0); }} className="btn-outline">
    ...
    <div className="card p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input {...register("prenom")} className="input-field" placeholder="ex. Marie" />
        ...
        <div>
          ...
          <input {...register("nom")} className="input-field" placeholder="ex. Dupont" />
      ...
      <div>
        ...
        <input {...register("email")} type="email" className="input-field" placeholder="prenom.nom@etablissement.fr" />
  ...
  const sectionAcces = (
    <div className="card p-6 space-y-5">
      ...
      <div className="form-pwd-grid gap-4">
        <div>
          <label className="block text-sm text-gray-300 mb-1.5">
        ...
        <div>
          ...
          <input {...register("confirmMotDePasse")} type="password" className="input-field" placeholder="••••••••••" />
          {errors.confirmMotDePasse && <p className="text-red-400 text-xs mt-1">{errors.confirmMotDePasse.message}</p>}
        </div>
      </div>
      <div>
        ...
        <select
          className="role-select-mobile w-full bg-navy-border text-white border border-navy-border rounded-lg px-4 py-3 text-sm"
          value={selectedRole}
        ...
        <div className="role-grid-desktop gap-3">
          {ROLES.map((r) => (
            <button
              key={r.value}
              type="button"
  ...
  const sectionEtab = (
    <div className="card p-6 space-y-5">
      ...
      <div>
        <label className="block text-sm text-gray-300 mb-1.5">Département / Formation</label>
        <input {...register("departement")} className="input-field" placeholder="ex. Développement Web" />
      </div>
      <div>
        <label className="block text-sm text-gray-300 mb-1.5">
          Numéro étudiant <span className="text-gray-500">(optionnel)</span>
        </label>
        <input {...register("numeroEtudiant")} className="input-field" placeholder="ex. 2023-WEB-042" />
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-navy">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        {/* Header */}
        <div className="mb-8">
          <button onClick={() => navigate(-1)} className="btn-outline flex items-center gap-2 mb-4 stepper-desktop">
            <HiArrowLeft /> Retour
          </button>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="stepper-mobile">
              ...
              {STEPS.map((label, i) => (
                <div key={i} className="flex items-center">
                  <div className="flex flex-col items-center gap-1">
                    <div className={`flex items-center justify-center w-9 h-9 rounded-full text-sm font-bold border-2 transition-colors ${
                      i < mobileStep
                      ...
                      {i < mobileStep ? <HiCheckCircle /> : i + 1}
                    </div>
                    <span className={`text-xs font-medium text-center ${i === mobileStep ? "text-white" : "text-gray-500"}`}>
                      {label}
                    </span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`w-10 h-0.5 mb-4 mx-1 ${i < mobileStep ? "bg-lime" : "bg-navy-border"}`} />
                  )}
                </div>
              ))}
            </div>
            ...
            {/* Contenu de l'étape */}
            <div className="mb-6">
              {mobileStep === 0 && sectionIdentite}
              {mobileStep === 1 && sectionAcces}
              {mobileStep === 2 && sectionEtab}
            </div>
            ...
            <div className="flex items-center justify-center gap-3">
              {mobileStep === 0 ? (
                ...
                <button type="button" onClick={() => setMobileStep((s) => s - 1)} className="btn-outline flex items-center gap-2">
                  <HiArrowLeft /> Précédent
                </button>
              )}
              {mobileStep < 2 ? (
                <button type="button" onClick={nextStep} className="btn-lime flex items-center gap-2">
                  Suivant <HiChevronRight />
                </button>
              ) : (
                <button type="submit" className="btn-lime flex items-center gap-2">
          ...
          <div className="stepper-desktop space-y-6">
            {sectionIdentite}
            {sectionAcces}
            {sectionEtab}
            <div className="flex items-center justify-center gap-3">
